...
const checkTriggerEditorUIValues = (trigger) => {
  ...
  cy.get('#triggerCondition')
    .scrollIntoView()
    .should('be.visible')
    .contains(triggerConditionToLabel[trigger.simple_triggers[0].on])
    .should('be.selected');

  const isDataTrigger = trigger.simple_triggers[0].type === 'data_trigger';
  ...
  if (trigger.simple_triggers[0].device_id) {
    ...
    cy.get('#triggerDeviceId')
      .scrollIntoView()
      .should('be.visible')
      .and('have.value', trigger.simple_triggers[0].device_id);
  } else if (trigger.simple_triggers[0].group_name) {
    cy.get('#triggerTargetSelect')
      .scrollIntoView()
      .should('be.visible')
      .contains('Group')
    ...
    cy.get('#triggerGroupName')
      .scrollIntoView()
      .should('be.visible')
      .and('have.value', trigger.simple_triggers[0].group_name);
  } else {
    cy.get('#triggerTargetSelect')
      .scrollIntoView()
      .should('be.visible')
  ...
  if (isDataTrigger) {
    const iface = trigger.simple_triggers[0].interface_name;
    if (iface === '*') {
      ...
      cy.get('#triggerInterfaceMajor')
        .scrollIntoView()
        .should('be.visible')
        .contains(trigger.simple_triggers[0].interface_major)
        .should('be.selected');
      cy.get('#triggerPath')
        .scrollIntoView()
        .should('be.visible')
        .and('have.value', trigger.simple_triggers[0].match_path);
      if (trigger.simple_triggers[0].value_match_operator) {
        cy.get('#triggerOperator')
          .scrollIntoView()
          .should('be.visible')
          .contains(triggerOperatorToLabel[trigger.simple_triggers[0].value_match_operator])
          .should('be.selected');
      }
      if (trigger.simple_triggers[0].known_value != null) {
        cy.get('#triggerKnownValue')
          .scrollIntoView()
          .should('be.visible')
          .and('have.value', trigger.simple_triggers[0].known_value);
      }
    }
  }
